...
# parse the top cards in each commander deck
my %reccount;
my %notinmylist;
open FILE, "harvest/percentdeckcount.txt" or die ("Couldn't open harvest/percentdeckcount.txt");
while (my $line = <FILE>) {
    chomp $line;
    next if ($line eq '');
    if ($line =~ /^(.*?)\t(.*)$/) {
...
# compile the counts from CubeTutor
my %cubecount;
my %onlyinmylist;
...
open FILE, ">results/percentdeckdiff.txt";
